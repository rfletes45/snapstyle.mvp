...
export type WreckPhase = "spawned" | "in_transit" | "docked";
...
export interface WreckState {
  ...
  // ── V2 fields (cutter-dock loop) ──
  /** Rarity tier for v2 wreck loop. Defaults to "common" for v1 wrecks. */
  rarity: WreckRarity;
  /** Phase in the cutter-dock pipeline. Defaults to "spawned" for v1 wrecks. */
  phase: WreckPhase;
  /** Flux payout when wreck is destroyed at the dock (micro-units). */
  payoutFluxMicro: number;
  /** Tick at which transit completes and wreck becomes docked. */
  dockReadyTick?: number;
  /** Tick until which a weakpoint is active (future PR hook). */
  weakpointUntilTick?: number;
}

// ─── Cutter Dock ─────────────────────────────────────────────

export interface CutterDockState {
  status: "empty" | "transit" | "docked";
  wreckId: string | null;
  reservedWreckId: string | null;
}

// ─── Run Stats (per-run, resets on rebirth) ──────────────────

export interface RunStats {
  totalCutTaps: number;
  totalFluxEarnedMicro: number;
  wrecksDestroyedTotal: number;
  wrecksDestroyedByRarity: Record<WreckRarity, number>;
}

// ─── Wreck Spawn Timer ──────────────────────────────────────

export interface WreckSpawnTimer {
...
export interface EventState {
  ...
  lastSpawnRollTick: number;
  /** V2: tick at which the next event may spawn (deterministic gap). */
  nextSpawnTick?: number;
...
// ─── Meta (persist across rebirths) ─────────────────────────

export interface LifetimeStats {
  totalWrecksDestroyed: number;
  totalFluxEarnedMicro: number;
}

export interface SimMeta {
  /** How many times the player has rebirthed. */
  rebirthCount: number;
  /** Currency for the rebirth tree. */
  coreShards: number;
  /** Branches unlocked by purchasing their keystone machine. */
  branchUnlocked: Record<string, boolean>;
  /** Rebirth-tree nodes purchased (node id → 1). Used to unlock locked slots. */
...
export interface SimStateV1 {
  ...
  /** Active wrecks. */
  wrecks: WrecksField;
  /** IDs of purchased upgrades (v1 legacy). */
  upgradesPurchased: string[];
  /** V2 upgrade purchased flags: upgradeId → 0|1. */
  ...
  // ── V2 cutter-dock loop ──
  /** Cutter dock state — drives the wreck → dock → cut → reward loop. */
  cutterDock: CutterDockState;
  /** Per-run cumulative stats (resets on rebirth). */
  runStats: RunStats;
  /** Wreck spawn timer. */
  wreckSpawn: WreckSpawnTimer;
  /** Persistent meta (survives rebirth). */
  meta: SimMeta;
}
...
export type InputCommand =
  ...
  | { t: "CUT_TAP"; atTick: number }
  | { t: "SELECT_WRECK"; wreckId: string; atTick: number }
  ...
  | { t: "BUY_UPGRADE_V2"; upgradeId: string; atTick: number }
  ...
  | { t: "CONFIRM_REBIRTH"; atTick: number }
  | { t: "BUY_REBIRTH_NODE"; nodeId: string; atTick: number }
