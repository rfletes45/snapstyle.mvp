...
export function getBallType(id: number): "cue" | "solid" | "stripe" | "eight" {
  if (id === 0) return "cue";
...
function groupToBallType(group: BallGroup): "solid" | "stripe" {
...
function countRemainingByGroup(balls: PoolBall[], group: BallGroup): number {
  const targetType = groupToBallType(group);
  let count = 0;
  for (const ball of balls) {
    if (ball.pocketed) continue;
    if (getBallType(ball.id) === targetType) count += 1;
  }
  return count;
}
...
function isMovingTowardPocket(ball: PoolBall, pocket: Pocket): boolean {
  ...
  return toPocketX * ball.vx + toPocketY * ball.vy > 0;
}
...
function integrateBall(ball: PoolBall): void {
  ...
  if (speed > 1e-9) {
    const decel = spinMagnitude > 0.2 ? SLIDING_FRICTION : ROLLING_FRICTION;
...
export function simulateShot(
  ...
  for (let step = 0; step < MAX_STEPS; step += 1) {
    for (const ball of world) integrateBall(ball);
    ...
    if (step % CAPTURE_STRIDE === 0) frames.push(cloneBalls(world));
    if (!world.some(hasMotion)) break;
  }
...
export function placeCueBall(balls: PoolBall[], x: number, y: number): PoolBall[] {
...
export function getLegalTargetIds(
  ...
  if (player.remaining <= 0) {
    return visible.filter((ball) => ball.id === 8).map((ball) => ball.id);
  }
...
function selectGroupFromOpenTable(pocketedIds: number[]): BallGroup | null {
  const solids = countPocketedByType(pocketedIds, "solid");
  const stripes = countPocketedByType(pocketedIds, "stripe");
  if (solids === 0 && stripes === 0) return null;
  if (solids > stripes) return "solids";
  if (stripes > solids) return "stripes";
...
export function evaluateShot(
  ...
  const firstContactType =
    shotResult.firstContact === null ? null : getBallType(shotResult.firstContact);
  ...
  if (shotResult.cueScratch) foulType = "scratch";
  else if (shotResult.firstContact === null) foulType = "no_contact";
  else if (shooterGroup) {
    const required = isShooterOnEight ? "eight" : groupToBallType(shooterGroup);
    if (firstContactType !== required) foulType = "wrong_ball_first";
  ...
  if (previousState.breakShot && !foulType) {
    const legalBreak = shotResult.railContacts >= 4 || pocketedObjectBalls.length > 0;
    if (!legalBreak) foulType = "no_rail";
  }
  ...
  if (foulType) shooterState.fouls += 1;
  else shooterState.fouls = 0;
  ...
  return { nextState, foulType, keepTurn, winner, message };
}
