...
): Omit<MatchedUserResult, "matchType"> {
  const d = doc.data()!;
  return {
    uid: doc.id,
    username: d.username ?? "",
...
async function getExclusionSets(callerUid: string) {
  const friendUidsSet = new Set<string>();
  ...
  friendsSnap.forEach((fdoc) => {
    const users = fdoc.data().users as string[];
    users.forEach((u) => {
      if (u !== callerUid) friendUidsSet.add(u);
    });
  });
...
// ═══════════════════════════════════════════════════════════════════════════
// 1. syncContacts — Upload + match + record reciprocal relationships
// ═══════════════════════════════════════════════════════════════════════════
...
interface ScoredMatch extends MatchedUserResult {
  reciprocal: boolean;
...
export const syncContacts = functions
  ...
  .https.onCall(
    async (
      ...
      if (
        ...
        phones.some((p: any) => typeof p !== "string") ||
        emails.some((e: any) => typeof e !== "string")
      ) {
        throw new functions.https.HttpsError(
          "invalid-argument",
      ...
      for (const p of phones) allHashes.push(sha256(p.trim()));
      for (const e of emails) allHashes.push(sha256(e.trim().toLowerCase()));
      ...
      for (let i = 0; i < phones.length; i += 10) {
        ...
        try {
          ...
          snap.forEach((sdoc) => {
            const d = sdoc.data();
            if (sdoc.id === callerUid) return;
            if (d.contactDiscovery?.discoverableViaContacts === false) return;
      ...
      if (recipCount > 0) await recipBatch.commit();
      ...
      const contactedBySnap = await db
        ...
        .collection("contactedBy")
        .get();
      const contactedByUids = new Set<string>();
      ...
      for (const matched of matchedUsers) {
        if (exclusions.allExcluded.has(matched.uid)) continue;
        try {
          ...
          theirFriendsSnap.forEach((fDoc) => {
            const users = fDoc.data().users as string[];
            const other = users.find((u: string) => u !== matched.uid);
            if (other && callerFriendUids.has(other)) mc++;
          });
      ...
      const rankedResults: ScoredMatch[] = [];
      for (const matched of matchedUsers) {
        if (exclusions.allExcluded.has(matched.uid)) continue;
        const isReciprocal = contactedByUids.has(matched.uid);
        const mutualCount = mutualCounts.get(matched.uid) ?? 0;
        let score = 3;
        if (isReciprocal) score += 2;
        score += mutualCount;
        const tags: string[] = ["In your contacts"];
        if (isReciprocal) tags.push("Has you in contacts");
        if (mutualCount > 0)
          tags.push(
...
// ═══════════════════════════════════════════════════════════════════════════
// 2. getContactRecommendations — Fetch ranked recommendations
// ═══════════════════════════════════════════════════════════════════════════
...
export const getContactRecommendations = functions.https.onCall(
  async (_data: unknown, context): Promise<GetRecommendationsResponse> => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "Must be signed in.",
    ...
    const contactedBySnap = await db
      ...
      .collection("contactedBy")
      .get();
    const recommendations = new Map<string, ContactRecommendation>();

    for (const cbDoc of contactedBySnap.docs) {
      const otherUid = cbDoc.id;
      if (exclusions.allExcluded.has(otherUid)) continue;
      try {
        ...
        if (otherData.phone || otherData.email) {
          ...
          if (otherData.phone) hashesToCheck.push(sha256(otherData.phone));
          if (otherData.email)
        ...
        try {
          ...
          theirFriends.forEach((fDoc) => {
            const users = fDoc.data().users as string[];
            const other = users.find((u: string) => u !== otherUid);
            if (other && callerFriendUids.has(other)) mutualCount++;
          });
        } catch {
          /* skip */
        }
        ...
        let score = 2;
        if (isReciprocal) score += 3;
        score += mutualCount;
        const tags: string[] = ["Has you in contacts"];
        if (isReciprocal) tags.unshift("In your contacts");
        if (mutualCount > 0)
          tags.push(
            `${mutualCount} mutual friend${mutualCount > 1 ? "s" : ""}`,
          );
...
export const removeSyncedContacts = functions.https.onCall(
  async (_data: unknown, context): Promise<{ success: boolean }> => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "Must be signed in.",
    ...
    await deleteSubcollection(
      db.collection("Users").doc(callerUid).collection("syncedContactHashes"),
    );
    await db
      .collection("Users")
      .doc(callerUid)
...
// ═══════════════════════════════════════════════════════════════════════════
// 4. updateContactDiscoverySettings — Privacy toggles
// ═══════════════════════════════════════════════════════════════════════════
...
export const updateContactDiscoverySettings = functions.https.onCall(
  async (
...
// ═══════════════════════════════════════════════════════════════════════════
// 5. matchContacts — Legacy callable (backward compat)
// ═══════════════════════════════════════════════════════════════════════════
...
export const matchContacts = functions.https.onCall(
  async (data: ContactMatchRequest, context): Promise<ContactMatchResponse> => {
    ...
    const exclusions = await getExclusionSets(callerUid);
    return {
      onAppUsers: matchedUsers.filter(
        (u) => !exclusions.allExcluded.has(u.uid),
      ),
      alreadyFriendUids: Array.from(exclusions.friendUids),
      pendingSentUids: Array.from(exclusions.pendingSent),
      pendingReceivedUids: Array.from(exclusions.pendingReceived),
