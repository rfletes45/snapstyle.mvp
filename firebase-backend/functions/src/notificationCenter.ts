...
async function chooseNotificationDecision(
  ...
  const foregroundSessions = sessions.filter(
    (session) =>
      FOREGROUND_APP_STATES.has(session.appState) &&
      !excludeDeviceIdSet.has(session.deviceId),
  );

  if (
    foregroundSessions.some((session) =>
  ...
  if (foregroundSessions.length > 0) {
    ...
    const activeForeground = foregroundSessions.filter(
      (s) => s.appState === "active",
    );
  ...
  // senderDeviceId.
  const filteredDevices = pushDevices.filter(
    (d) =>
      !excludeDeviceIdSet.has(d.deviceId) &&
