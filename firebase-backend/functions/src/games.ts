...
const REMOVED_GAME_IDS = new Set<string>([]);
...
interface UniversalGameInvite {
  ...
  // ── Finalization / visibility fields (Phase 1 hardening) ──
  resolvedAt?: number;
 ...
 */
function getInitialGameState(gameType: TurnBasedGameType): any {
  switch (gameType) {
    case "chess":
      return {
...
function getInitialCheckersBoard(): (string | null)[][] {
 ...
 */
export const createGameFromInvite = functions.firestore
  .document("GameInvites/{inviteId}")
  .onUpdate(async (change, context) => {
    const before = change.before.data() as GameInvite;
    const after = change.after.data() as GameInvite;
    ...
    if (before.status === "pending" && after.status === "accepted") {
      ...
      try {
        ...
        const senderRating =
          senderStats?.gameStats[invite.gameType]?.rating ?? DEFAULT_RATING;
        const receiverRating =
          receiverStats?.gameStats[invite.gameType]?.rating ?? DEFAULT_RATING;
 ...
 */
export const processGameCompletion = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (wasActive && isTerminal) {
      try {
        ...
        const gameEndStatus = after.status as string;
        ...
        if (after.inviteId) {
          ...
          await finalizeInviteByGameId(
            context.params.gameId,
            resolutionType,
            after.winner?.playerId,
          );
        }
...
export const processRealtimeGameCompletion = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    // gameId-based search.
    try {
      let sessionInviteId: string | undefined = data.inviteId;
      const sessionGameId: string | undefined =
 ...
 */
async function finalizeInviteByGameId(
  gameId: string,
  resolutionType: string,
  winnerId?: string,
): Promise<void> {
...
async function finalizeUniversalInvite(
  ...
  try {
    const result = await db.runTransaction(async (tx) => {
      ...
      const invite = snap.data() as UniversalGameInvite;
 ...
 */
export const onGameCompletedCreateHistory = functions.firestore
  .document("TurnBasedGames/{gameId}")
  .onUpdate(async (change, context) => {
    const before = change.before.data() as TurnBasedGame;
    const after = change.after.data() as TurnBasedGame;
    ...
    try {
      ...
      if (after.status === "abandoned") {
        ...
        endReason = "timeout";
      } else if ((after as any).status === "draw") {
      ...
      const player1Moves = moves.filter(
        (m: { playerId?: string }) => m.playerId === player1.id,
      ).length;
      const player2Moves = moves.filter(
        (m: { playerId?: string }) => m.playerId === player2.id,
      ).length;
      ...
      const historyRecord = {
        ...
        conversationId: after.inviteId
          ? undefined
          : (after as any).conversationId, // Will be set when context tracking is added
        conversationType: (after as any).conversationType,
        startedAt,
        completedAt,
        duration,
 ...
 */
export const onGameHistoryCreatedUpdateLeaderboard = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    functions.logger.info(
      `Updating LeaderboardStats for history ${historyId}`,
      {
    ...
    try {
      const batch = db.batch();
      const now = Timestamp.now();
      const gameType = history.gameType as TurnBasedGameType;
      ...
      for (const player of history.players || []) {
        ...
        const gameTypes: (TurnBasedGameType | "all")[] = [gameType, "all"];
        ...
        for (const gt of gameTypes) {
          for (const timeframe of timeframes) {
            ...
            const newWinRate =
              newGamesPlayed > 0 ? (newWins / newGamesPlayed) * 100 : 0;
      ...
      functions.logger.info(
        `LeaderboardStats updated for history ${historyId}`,
        {
          gameType: history.gameType,
          players: history.players?.map((p: { userId: string }) => p.userId),
        },
      );
    } catch (error) {
      functions.logger.error(
        `Failed to update LeaderboardStats for ${historyId}`,
        {
          error,
          gameType: history.gameType,
        },
 ...
 */
async function updatePlayerStats(game: TurnBasedGame): Promise<void> {
  ...
  if (game.settings.isRated) {
    ...
    const player1Score =
      player1Outcome === "win" ? 1 : player1Outcome === "draw" ? 0.5 : 0;
    const player2Score = 1 - player1Score;

    player1NewRating = calculateNewRating(
      player1.rating ?? DEFAULT_RATING,
      player1Expected,
      player1Score,
      32,
    ...
    player2NewRating = calculateNewRating(
      player2.rating ?? DEFAULT_RATING,
      player2Expected,
      player2Score,
      32,
...
async function updateSinglePlayerStats(
  ...
  await db.runTransaction(async (transaction) => {
    ...
    let data: PlayerGameStatsDoc;

    if (doc.exists) {
      data = doc.data() as PlayerGameStatsDoc;
    } else {
      data = {
        playerId,
        gameStats: {},
 ...
 */
async function checkAchievements(game: TurnBasedGame): Promise<void> {
...
async function checkPlayerAchievements(
  ...
  for (const check of gameCountChecks) {
    await updateAchievementProgress(
      achievements,
      check.id,
      statsDoc.overall.totalGamesPlayed,
      check.threshold,
    );
  }
  ...
  for (const check of ratingChecks) {
    await updateAchievementProgress(
      achievements,
      check.id,
      gameStats.peakRating ?? DEFAULT_RATING,
      check.threshold,
    );
  }

  // Save achievements
  achievements.updatedAt = Timestamp.now();
...
function updateAchievementProgress(
  ...
  progress.percentComplete = Math.min(
    100,
    (progress.currentValue / threshold) * 100,
  );
 ...
 */
export const processMatchmakingQueue = functions.pubsub
  .schedule("every 1 minutes")
  .onRun(async () => {
    const gameTypes: TurnBasedGameType[] = [
      "chess",
      "checkers",
      "tic_tac_toe",
      "crazy_eights",
 ...
 */
async function processGameTypeQueue(
  gameType: TurnBasedGameType,
): Promise<void> {
  const now = Timestamp.now();
  ...
  for (let i = 0; i < entries.length; i++) {
    if (matched.has(entries[i].data.id)) continue;

    const player1 = entries[i];
    const player1Seconds =
      (now.toMillis() - player1.data.createdAt.toMillis()) / 1000;
    const player1Range = calculateCurrentRange(
      player1.data.initialRatingRange,
      player1.data.maxRatingRange,
      player1.data.rangeExpansionRate,
      player1Seconds,
    );
    ...
    for (let j = i + 1; j < entries.length; j++) {
      if (matched.has(entries[j].data.id)) continue;

      const player2 = entries[j];
      const player2Seconds =
        (now.toMillis() - player2.data.createdAt.toMillis()) / 1000;
      const player2Range = calculateCurrentRange(
        player2.data.initialRatingRange,
        player2.data.maxRatingRange,
        player2.data.rangeExpansionRate,
        player2Seconds,
      );

      // Check if rated preference matches
      if (player1.data.isRated !== player2.data.isRated) continue;

      // Check if within range
      const ratingDiff = Math.abs(player1.data.rating - player2.data.rating);
      if (ratingDiff > player1Range || ratingDiff > player2Range) continue;
...
async function createMatchFromQueue(
  player1Entry: { ref: FirebaseFirestore.DocumentReference; data: any },
  player2Entry: { ref: FirebaseFirestore.DocumentReference; data: any },
  gameType: TurnBasedGameType,
): Promise<void> {
  const now = Timestamp.now();
  const gameId = `game_${Date.now().toString(36)}_${Math.random().toString(36).substring(2, 8)}`;
 ...
 */
export const expireGameInvites = functions.pubsub
    ...
    for (const status of EXPIRABLE_STATUSES) {
      ...
      for (const inviteDoc of snapshot.docs) {
        try {
          ...
          functions.logger.warn(
            "[expireGameInvites] Finalize failed for invite",
            {
              inviteId: inviteDoc.id,
              error: err instanceof Error ? err.message : String(err),
            },
 ...
 */
export const expireMatchmakingEntries = functions.pubsub
 ...
 */
export const cleanupVacantGames = functions.pubsub
  .schedule("every 5 minutes")
  .onRun(async () => {
    const now = Date.now();
    ...
    for (const doc of snapshot.docs) {
      ...
      if (tbDoc.exists) batch.delete(tbDoc.ref);

      const rtDoc = await db
        .collection("RealtimeGameSessions")
        .doc(gameId)
        .get();
      if (rtDoc.exists) batch.delete(rtDoc.ref);
 ...
 */
export const reconcileActiveInvites = functions.pubsub
  ...
  .onRun(async () => {
    ...
    try {
      ...
      if (activeSnap.size === ACTIVE_LIMIT) {
        functions.logger.warn(
          "[reconcileActiveInvites] Active query hit limit — more stuck invites may exist",
          { limit: ACTIVE_LIMIT },
        );
      }
      ...
      for (const inviteDoc of activeSnap.docs) {
        ...
        if (!updatedAt && !createdAt) {
          functions.logger.warn(
            "[reconcileActiveInvites] Invite has no valid timestamps — treating as stuck",
            { inviteId: inviteDoc.id },
          );
        }
        ...
        if (invite.gameId) {
          const gameIdStr = invite.gameId as string;
        ...
        if (gameAlive) continue; // game is still running — don't touch

        // Game is gone or completed but invite is stuck active → finalize
        functions.logger.warn(
          "[reconcileActiveInvites] Finalizing stuck active invite",
          {
            inviteId: inviteDoc.id,
            gameType,
        ...
        if (result.success) reconciledActive++;
      }
    } catch (err) {
    ...
    try {
      ...
      if (startingSnap.size === STARTING_LIMIT) {
        functions.logger.warn(
          "[reconcileActiveInvites] Starting query hit limit — more stuck invites may exist",
          { limit: STARTING_LIMIT },
        );
      }
      ...
      for (const inviteDoc of startingSnap.docs) {
        ...
        if (startingAge < STARTING_STUCK_MS) continue;

        functions.logger.warn(
          "[reconcileActiveInvites] Cancelling stuck starting invite",
          {
            inviteId: inviteDoc.id,
            gameType: invite.gameType,
        ...
        if (result.success) reconciledStarting++;
      }
    } catch (err) {
...
export const makeMove = functions.https.onCall(async (data, context) => {
  ...
  try {
    const result = await db.runTransaction(async (transaction) => {
      ...
      const game = gameDoc.data() as TurnBasedGame;
      ...
      const now = Timestamp.now();
      const nextTurn =
        game.currentTurn === game.players.player1.id
 ...
 * Resign from a game
 */
export const resignGame = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "Must be logged in",
  ...
  try {
    await db.runTransaction(async (transaction) => {
      ...
      const winnerId =
        game.players.player1.id === context.auth!.uid
          ? game.players.player2.id
          : game.players.player1.id;
...
// =============================================================================
// Universal Game Result Pipeline — onGameResult
// =============================================================================

/**
 * XP rules (mirrored from src/types/gameResult.ts for server authority).
 * Any change here must be kept in sync with the client copy.
 */
type GameXpCategory =
  | "arcade"
  | "puzzle"
...
const XP_BASE: Record<GameXpCategory, number> = {
  arcade: 15,
  puzzle: 20,
  board: 25,
  card: 20,
...
const XP_OUTCOME_MULTIPLIER: Record<GameResultOutcome, number> = {
  win: 2.0,
  completed: 1.5,
  draw: 1.2,
  lose: 0.5,
...
const GAME_XP_CATEGORY: Record<string, GameXpCategory> = {
  bounce_blitz: "arcade",
  play_2048: "puzzle",
  word_master: "daily",
...
interface GameResultParticipant {
  userId: string;
  displayName: string;
...
export async function awardGameXp(
  ...
  let totalXp = 0;
  let previousLevel = 1;
  let newLevelInfo: ReturnType<typeof calculateLevelFromXp>;
...
export const onGameResult = functions.https.onCall(
  async (data: GameResultEvent, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "Must be logged in",
    ...
    try {
      ...
      const gameSpecific: Record<string, number> = {
...
export const claimLevelReward = functions.https.onCall(
  async (data: { level: number }, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "Must be logged in",
    ...
    if (
      ...
      throw new functions.https.HttpsError(
        "invalid-argument",
        `level must be an integer between 1 and ${MAX_REWARD_LEVEL}`,
      );
    }
    ...
    const totalXp: number = userData.gameXp ?? 0;
    const levelInfo = calculateLevelFromXp(totalXp);

    if (levelInfo.level < level) {
      throw new functions.https.HttpsError(
        "failed-precondition",
        `You are level ${levelInfo.level}; level ${level} not yet reached`,
      );
    }

    // ── Check not already claimed ──────────────────────────────────
    const claimedLevels: number[] = userData.claimedLevels ?? [];
    if (claimedLevels.includes(level)) {
      throw new functions.https.HttpsError(
        "already-exists",
        `Level ${level} reward already claimed`,
      );
    }
    ...
    batch.update(userRef, {
      claimedLevels: FieldValue.arrayUnion(level),
      cosmeticPoints: FieldValue.increment(amount),
    });
