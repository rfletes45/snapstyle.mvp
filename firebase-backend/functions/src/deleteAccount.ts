...
interface DeletionJobData {
  ...
  completedAt: admin.firestore.FieldValue | admin.firestore.Timestamp | null;
}
...
async function deleteQueryBatched(
  ...
  while (!snapshot.empty) {
    ...
    functions.logger.info(
      `[deleteAccount] Deleted ${snapshot.size} docs from ${label}`,
    );
...
async function deleteStoragePrefix(prefix: string): Promise<number> {
  try {
        ...
        file.delete().catch((err: any) => {
          if (err?.code !== 404) {
            functions.logger.warn(
              `[deleteAccount] Failed to delete storage file ${file.name}:`,
              err.message,
            );
          }
        }),
    ...
    functions.logger.info(
      `[deleteAccount] Deleted ${files.length} files from storage prefix: ${prefix}`,
    );
    return files.length;
  } catch (err: any) {
    if (err?.code !== 404) {
      functions.logger.warn(
        `[deleteAccount] Storage prefix delete error for ${prefix}:`,
        err.message,
      );
    }
    return 0;
 ...
 */
async function redactUserInDoc(
  ...
  try {
    ...
    functions.logger.warn(
      `[deleteAccount] Failed to redact ${docRef.path}:`,
      err.message,
    );
  }
}
...
async function removeFromArray(
  ...
  try {
    ...
    if (err?.code !== 5) {
      ...
      functions.logger.warn(
        `[deleteAccount] Failed to remove from array ${docRef.path}.${arrayField}:`,
        err.message,
      );
    }
  }
...
async function step_deleteUserSubcollections(uid: string): Promise<void> {
...
async function step_deleteUserProfileDoc(uid: string): Promise<void> {
  const userRef = db.collection("Users").doc(uid);
...
async function step_releaseUsername(uid: string): Promise<string | null> {
  ...
  let releasedUsername: string | null = null;

  for (const doc of usernamesSnap.docs) {
    releasedUsername = doc.id;
    await doc.ref.delete();
    functions.logger.info(
      `[deleteAccount] Released username: ${releasedUsername}`,
    );
  }
...
async function step_deleteWallet(uid: string): Promise<void> {
...
async function step_deleteTransactions(uid: string): Promise<void> {
  const q = db.collection("Transactions").where("uid", "==", uid);
  await deleteQueryBatched(q, "Transactions");
}

async function step_deleteFriends(uid: string): Promise<void> {
  const q = db.collection("Friends").where("users", "array-contains", uid);
  await deleteQueryBatched(q, "Friends");
}

async function step_deleteFriendRequests(uid: string): Promise<void> {
...
async function step_cleanupChats(uid: string): Promise<void> {
  ...
  for (const chatDoc of chatsSnap.docs) {
    ...
    if (otherMembers.length === 0) {
      ...
      functions.logger.info(
        `[deleteAccount] Deleted empty chat: ${chatDoc.id}`,
      );
    } else {
      ...
      const messageBatches: admin.firestore.WriteBatch[] = [];
      ...
      if (opCount > 0) messageBatches.push(currentBatch);
...
async function step_cleanupGroups(uid: string): Promise<void> {
  ...
  for (const groupDoc of groupsSnap.docs) {
    ...
    const otherMembers = memberIds.filter((m: string) => m !== uid);
    ...
    if (otherMembers.length === 0 && groupData.ownerId === uid) {
      ...
      functions.logger.info(
        `[deleteAccount] Deleted empty group: ${groupDoc.id}`,
      );
    } else {
...
async function step_deleteGroupInvites(uid: string): Promise<void> {
...
async function step_deleteStories(uid: string): Promise<void> {
...
async function step_deletePictures(uid: string): Promise<void> {
...
async function step_deleteConversations(uid: string): Promise<void> {
    ...
    for (const field of ["participantIds", "members", "participants"]) {
      try {
        ...
        for (const convDoc of snap.docs) {
          ...
          const participants: string[] = data[field] || [];
...
async function step_deleteNotifications(uid: string): Promise<void> {
...
async function step_deleteScheduledMessages(uid: string): Promise<void> {
  const q = db.collection("ScheduledMessages").where("senderId", "==", uid);
  await deleteQueryBatched(q, "ScheduledMessages");
}

async function step_cleanupCalls(uid: string): Promise<void> {
...
async function step_deleteGroupCallInvites(uid: string): Promise<void> {
  const qInviter = db
...
async function step_deletePurchases(uid: string): Promise<void> {
...
async function step_cleanupGifts(uid: string): Promise<void> {
...
async function step_deleteGameData(uid: string): Promise<void> {
  ...
  for (const invDoc of invitesSnap.docs) {
    ...
    const others = participants.filter((p: string) => p !== uid);
  ...
  for (const gameDoc of leaderboardGamesSnap.docs) {
    ...
    for (const weekDoc of weeksSnap.docs) {
      ...
      if (entrySnap.exists) {
        ...
        functions.logger.info(
          `[deleteAccount] Deleted leaderboard entry: ${entryRef.path}`,
        );
      }
    }
  }
...
async function step_deleteAnalytics(uid: string): Promise<void> {
  // Analytics events
  ...
  for (const field of ["memberIds", "members", "adminIds"]) {
    try {
      ...
      for (const doc of snap.docs) {
        ...
        const data = doc.data();
        if (data.ownerId === uid) {
          const others = (data.memberIds || data.members || []).filter(
            (m: string) => m !== uid,
          );
          if (others.length > 0) {
...
async function step_deleteStorage(uid: string): Promise<void> {
  ...
  for (const folderPrefix of ["pictures/", "dm-voice/", "snaps/"]) {
    try {
      ...
      if (userFiles.length > 0) {
        functions.logger.info(
          `[deleteAccount] Deleted ${userFiles.length} files from ${folderPrefix}`,
        );
      }
    } catch (err: any) {
      functions.logger.warn(
        `[deleteAccount] Error cleaning ${folderPrefix}:`,
        err.message,
      );
    }
  }
...
async function step_deleteRealtimeDatabase(uid: string): Promise<void> {
  // Presence
  await rtdb
    .ref(`presence/${uid}`)
    .remove()
    .catch((err: any) => {
      functions.logger.warn(
        `[deleteAccount] RTDB presence cleanup error:`,
        err.message,
      );
    });
  ...
  await rtdb
    ...
    .catch((err: any) => {
      functions.logger.warn(
        `[deleteAccount] RTDB statusVisibility cleanup error:`,
        err.message,
      );
    });
}

async function step_deleteAuthUser(uid: string): Promise<void> {
  try {
...
export const deleteAccountFunction = functions
  .runWith({
  ...
  .https.onCall(
    async (
      ...
      functions.logger.info(
        `[deleteAccount] Starting account deletion for uid=${uid}, email=${email}`,
      );
      ...
      if (existingJob.exists) {
        const existing = existingJob.data() as DeletionJobData;
        ...
        stepsCompleted = existing.stepsCompleted || [];
        functions.logger.info(
          `[deleteAccount] Resuming deletion job. Previously completed: ${stepsCompleted.join(", ")}`,
        );
      }
      ...
      await jobRef.set(
        {
          ...
          startedAt: existingJob.exists
            ? (existingJob.data() as DeletionJobData).startedAt
      ...
      for (const step of steps) {
        if (stepsCompleted.includes(step.name)) {
          functions.logger.info(
            `[deleteAccount] Skipping already-completed step: ${step.name}`,
          );
          continue;
        }
      ...
      functions.logger.info(
        `[deleteAccount] Deletion ${allSucceeded ? "completed" : "completed with errors"} for uid=${uid}. ` +
