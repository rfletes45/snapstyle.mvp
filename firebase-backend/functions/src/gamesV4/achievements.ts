...
interface EvaluationContext {
  ...
  /** Set of achievement types already earned by this player. */
...
const GAME_ACHIEVEMENTS: AchievementDef[] = [
  ...
  // ═══════════════════════════════════════════════════════════════════════
  {
    ...
    tokenReward: 5,
    evaluate: (ctx) => {
      if (ctx.gameId !== "pong_game") return false;
      return (ctx.pbStats?.totalPlays ?? 0) <= 1;
    },
  },
  {
  ...
  },
  {
    ...
    evaluate: (ctx) => {
      if (ctx.gameId !== "pong_game") return false;
      if (!ctx.winnerIds.includes(ctx.uid)) return false;
      const pm = ctx.performanceMetrics as Record<string, unknown>;
  ...
  },
  {
    ...
    evaluate: (ctx) => {
      ...
      const pm = ctx.performanceMetrics as Record<string, unknown>;
      return (
        typeof pm?.longestRallyHits === "number" && pm.longestRallyHits >= 20
      );
    },
  },
  {
    ...
    evaluate: (ctx) => {
      ...
      const pm = ctx.performanceMetrics as Record<string, unknown>;
      return (
        typeof pm?.fastestPointMs === "number" && pm.fastestPointMs <= 3000
      );
    },
  },
  {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  {
    ...
    tokenReward: 5,
    evaluate: (ctx) => {
      if (ctx.gameId !== "knockout_game") return false;
      return (ctx.pbStats?.totalPlays ?? 0) <= 1;
    },
  },
  {
  ...
  },
  {
    ...
    evaluate: (ctx) => {
      ...
      const pm = ctx.performanceMetrics as Record<string, unknown>;
      return (
        typeof pm?.shrinkStagesSurvived === "number" &&
  ...
  },
  {
    ...
    tokenReward: 10,
    evaluate: (ctx) => {
      if (ctx.gameId !== "dead_drop") return false;
      return ctx.winnerIds.includes(ctx.uid);
    },
  },
  {
    ...
    evaluate: (ctx) => {
      if (ctx.gameId !== "dead_drop") return false;
      const me = (
        ctx.performanceMetrics?.perPlayer as Record<
          string,
          Record<string, unknown>
        >
      )?.[ctx.uid];
  ...
  },
  {
    ...
    evaluate: (ctx) => {
      ...
      if (!me || !me.won) return false;
      // Check if the other role's win achievement is already earned
      // (this current game provides one role win, the other must exist)
      const hasSpymasterWin =
        ctx.earnedAchievements.has("dd_win_as_spymaster") ||
        me.wonAsSpymaster === true;
      const hasOperativeWin =
        ctx.earnedAchievements.has("dd_win_as_operative") ||
  ...
  },
  {
    ...
    evaluate: (ctx) => {
      ...
      if (!me) return false;
      return (me.maxCorrectFromSingleClue as number) >= 5;
    },
  },
  {
    type: "dd_clean_win",
  ...
  },
  {
    ...
    evaluate: (ctx) => {
      ...
      if (!me) return false;
      return me.wonAsOperative === true && (me.wrongGuesses as number) === 0;
    },
  },
  {
    type: "dd_play_20",
  ...
  // ── Metro Magnate ────────────────────────────────────────────────────────
  {
    ...
    tokenReward: 5,
    evaluate: (ctx) => {
      if (ctx.gameId !== "metro_magnate") return false;
      return (ctx.pbStats?.totalPlays ?? 0) >= 1;
    },
  },
  {
  ...
  },
  {
    ...
    evaluate: (ctx) => {
      if (ctx.gameId !== "metro_magnate") return false;
      const pp = (
        ctx.performanceMetrics?.perPlayer as
          | Record<string, Record<string, unknown>>
          | undefined
      )?.[ctx.uid];
      return ((pp?.sectorsCompleted as number) ?? 0) >= 1;
    },
  },
  {
    type: "mm_build_tower",
    ...
    evaluate: (ctx) => {
      ...
      return ((pp?.towersBuilt as number) ?? 0) >= 1;
    },
  },
  {
    ...
    evaluate: (ctx) => {
      ...
      return ((pp?.transitLinesOwned as number) ?? 0) >= 4;
    },
  },
  {
    ...
    evaluate: (ctx) => {
      ...
      return ((pp?.serviceNodesOwned as number) ?? 0) >= 2;
    },
  },
  {
    type: "mm_high_roller",
    ...
    evaluate: (ctx) => {
      ...
      return ((pp?.netWorth as number) ?? 0) >= 5000;
    },
  },
  {
    type: "mm_monopolist",
    ...
    evaluate: (ctx) => {
      ...
      return ((pp?.sectorsCompleted as number) ?? 0) >= 2;
    },
  },
  {
    type: "mm_express_win",
  ...
  },
  {
    ...
    evaluate: (ctx) => {
      ...
      return ((pp?.mortgagedCount as number) ?? 0) === 0;
    },
  },
  {
    type: "mm_win_5",
...
export function getAchievementRewardTransactionId(
  uid: string,
...
export function formatAchievementDisplayName(
  achievementType: string,
...
function normalizeTokenReward(value: unknown): number {
  return typeof value === "number" && Number.isFinite(value)
    ? Math.max(0, value)
    : 0;
}

function writeAchievementRewardTransaction(
  ...
  params: {
    ...
    previousClaimState: string | null;
  },
): void {
...
async function createAchievementWithAutoReward(params: {
  ...
  const outcome = await db.runTransaction(async (tx) => {
    ...
    if (achievementSnap.exists) {
      return { created: false, transactionId: null as string | null };
    }

    const transactionRef = db.collection("Transactions").doc(transactionId);
    const transactionSnap =
      tokenReward > 0 ? await tx.get(transactionRef) : null;
...
export async function awardExistingUnclaimedAchievementReward(params: {
  db: FirebaseFirestore.Firestore;
  uid: string;
  achievementType: string;
}): Promise<{
  ...
  return db.runTransaction(async (tx) => {
    ...
    const transactionSnap =
      tokenReward > 0 ? await tx.get(transactionRef) : null;
    const alreadyRewarded = !!transactionSnap?.exists;
    const achievementName = formatAchievementDisplayName(
      achievementType,
    ...
    for (const def of GAME_ACHIEVEMENTS) {
      ...
      try {
        if (def.evaluate(ctx)) {
          ...
          console.log(
            `[achievementsV4] ${uid} unlocked "${def.type}" (+${def.tokenReward} tokens auto-awarded) in session ${result.sessionId}`,
          );
        }
      } catch (err) {
        console.error(
