...
import { evaluateAchievementsV4 } from "./achievements";
import {
  computeOutcome,
...
import { notifyAchievementUnlocked, notifyResolved } from "./notifications";
import type {
  AchievementUnlock,
...
export async function resolveSessionV4Internal(
  ...
  const lbUpdates = await updateLeaderboards(
    db,
    session.gameId,
    weekKey,
    scoreboard,
...
async function updatePersonalBests(
  ...
  for (const entry of scoreboard) {
    ...
    try {
      ...
      if (entry.score > (currentPb as number)) {
