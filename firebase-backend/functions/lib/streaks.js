...
exports.updateStreakOnMessage = updateStreakOnMessage;
...
async function updateStreakOnMessage(senderId, recipientId) {
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        const otherLastSent = data[otherSentField] || "";
        const streakUpdatedDay = data.streakUpdatedDay || "";
        let streakCount = data.streakCount || 0;
        let bestCount = data.streakBestCount || streakCount;
        const graceUsedAt = data.streakGraceUsedAt || "";
        ...
        if (otherSentToday && streakUpdatedDay !== today) {
            ...
            if (!streakUpdatedDay) {
                ...
                if (gap <= 1) {
                    ...
                    console.log(`[streaks] Grace save used for friendship ${friendDoc.id}`);
                }
                else {
    ...
    if (result && result.milestoneReached) {
        ...
        await Promise.all([
            sendStreakMilestoneNotification(result.uid1, result.uid2, ms, result.friendshipId),
            grantMilestoneCosmetic(result.uid1, ms),
            grantMilestoneCosmetic(result.uid2, ms),
        ]);
        console.log(`[streaks] Milestone ${ms} reached for friendship ${result.friendshipId}`);
    }
}
// ─── Grace logic ──────────────────────────────────────────────────────────────
function canUseGrace(lastGrace, today) {
    if (!lastGrace)
        return true;
    return daysBetween(lastGrace, today) >= GRACE_COOLDOWN_DAYS;
}
// ─── Milestone notifications ──────────────────────────────────────────────────
async function sendStreakMilestoneNotification(user1Id, user2Id, milestone, friendshipId) {
...
exports.streakReminder = functions.pubsub
    ...
    try {
        ...
        console.log(`[streaks] Checking ${activeSnap.docs.length} active streaks`);
        for (const doc of activeSnap.docs) {
            try {
                ...
                if (gap > 2) {
                    ...
                    console.log(`[streaks] Reset zombie streak on ${doc.id} (gap=${gap})`);
                    continue;
                }
                ...
                if (gap >= 1) {
                    ...
                    const sendReminder = async (userToNotify) => {
                        await (0, notificationCenter_1.notifyUser)({
