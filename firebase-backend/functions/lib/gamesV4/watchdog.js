...
exports.watchdogGamesV4 = void 0;
...
// Every 30 minutes
// =============================================================================
exports.watchdogGamesV4 = functions.pubsub
    ...
    try {
        const cutoff = admin.firestore.Timestamp.fromMillis(now - types_1.LOBBY_EXPIRY_MS);
    ...
    try {
        ...
        console.error("[watchdogV4] Pass 2 (delete expired invites) failed:", err);
    }
    ...
    try {
        ...
        for (const doc of unrewardedSessions.docs) {
            ...
            try {
                await (0, resolve_1.retryRewardsForSession)(session.sessionId);
                pass3Count++;
            }
            catch (err) {
                console.error(`[watchdogV4] Pass 3 retry failed for ${session.sessionId}:`, err);
            }
        }
    }
    catch (err) {
    ...
    try {
        const inactivityCutoff = admin.firestore.Timestamp.fromMillis(now - TURN_INACTIVITY_MS);
        ...
        for (const doc of staleSessions.docs) {
            ...
            try {
                ...
                console.error(`[watchdogV4] Pass 4 auto-resolve failed for ${session.sessionId}:`, err);
            }
        }
    }
    catch (err) {
    ...
    console.log(`[watchdogV4] Complete. ` +
