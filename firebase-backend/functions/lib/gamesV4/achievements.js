...
exports.ACHIEVEMENT_SECTIONS = void 0;
    ...
    },
    {
        ...
        evaluate: (ctx) => {
            ...
            if (!ctx.winnerIds.includes(ctx.uid))
                return false;
            const opponents = ctx.scoreboard.filter((e) => e.uid !== ctx.uid);
            return opponents.every((e) => (e?.cardsLeft ?? 0) >= 5);
        },
    },
    {
            ...
            try {
                if (def.evaluate(ctx)) {
                    ...
                    if (def.tokenReward > 0) {
                        ...
                        batch.set(walletRef, {
                            tokensBalance: admin.firestore.FieldValue.increment(def.tokenReward),
                            totalEarned: admin.firestore.FieldValue.increment(def.tokenReward),
                        }, { merge: true });
                    }
                    await batch.commit();
                    unlocks.push({
                    ...
                    console.log(`[achievementsV4] ${uid} unlocked "${def.type}" (+${def.tokenReward} tokens) in session ${result.sessionId}`);
                }
            }
            catch (err) {
