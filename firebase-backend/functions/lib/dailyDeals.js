...
exports.triggerDailyDeals = exports.cleanupOldDeals = exports.generateWeeklyDeals = exports.generateDailyDeals = void 0;
 ...
 */
function pickRandomDiscount() {
    const totalWeight = DISCOUNT_TIERS.reduce((sum, tier) => sum + tier.weight, 0);
 ...
 */
function calculateDealPrice(originalPrice, discountPercent) {
 ...
 */
exports.generateDailyDeals = functions.pubsub
    ...
    try {
        ...
        const eligibleItems = allItems.filter((item) => {
            ...
            if (item.priceTokens < 50)
                return false;
            // Skip limited time items
            if (item.availableTo && item.availableTo < Date.now())
                return false;
            return true;
        });
        ...
        const selectedItems = shuffled.slice(0, Math.min(DEALS_PER_DAY, shuffled.length));
        ...
        selectedItems.forEach((item, index) => {
            const discountPercent = index === 0 ? FEATURED_DISCOUNT : pickRandomDiscount();
            ...
            console.log(`[dailyDeals] Deal ${index + 1}: ${item.name} - ${discountPercent}% off (${item.priceTokens} -> ${dealPrice})`);
        });
        // 6. Also set metadata on parent document
        batch.set(db.collection("DailyDeals").doc(dateString), {
            date: dateString,
        ...
        await batch.commit();
        console.log(`[dailyDeals] Successfully generated ${selectedItems.length} deals for ${dateString}`);
        return null;
    }
    catch (error) {
 ...
 */
exports.generateWeeklyDeals = functions.pubsub
    .schedule("0 0 * * 1") // Monday at midnight UTC
    .timeZone("UTC")
    .onRun(async () => {
    const db = admin.firestore();
    const weekStart = getWeekStartString();
    console.log(`[weeklyDeals] Generating deals for week starting ${weekStart}`);
    try {
        ...
        selectedItems.forEach((item, index) => {
            ...
            const dealRef = dealsCollectionRef.doc(`deal_${index + 1}`);
            batch.set(dealRef, dealData);
            console.log(`[weeklyDeals] Deal ${index + 1}: ${item.name} - ${discountPercent}% off`);
        });
        // Set metadata
        batch.set(db.collection("WeeklyDeals").doc(weekStart), {
            weekStart,
            dealsCount: selectedItems.length,
            startsAt,
            endsAt,
        ...
        await batch.commit();
        console.log(`[weeklyDeals] Successfully generated ${selectedItems.length} weekly deals`);
        return null;
    }
    catch (error) {
 ...
 */
exports.cleanupOldDeals = functions.pubsub
 ...
 */
exports.triggerDailyDeals = functions.https.onCall(async (data, context) => {
    // Verify admin
...
//# sourceMappingURL=dailyDeals.js.map