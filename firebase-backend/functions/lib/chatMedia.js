...
exports.cleanupStagingOrphans = exports.mintChatMediaUrl = void 0;
 ...
 */
async function commitStagedAttachments(scope, conversationId, messageId, staged) {
    // Validate count
    if (staged.length > MAX_ATTACHMENTS) {
        throw new functions.https.HttpsError("invalid-argument", `Maximum ${MAX_ATTACHMENTS} attachments per message`);
    }
    ...
    for (const att of staged) {
        ...
        if (!ALLOWED_MIME_PREFIXES.some((prefix) => att.mime.startsWith(prefix))) {
            throw new functions.https.HttpsError("invalid-argument", `Unsupported MIME type: ${att.mime}`);
        }
        // Validate size
        if (att.sizeBytes > MAX_FILE_SIZE_BYTES) {
            throw new functions.https.HttpsError("invalid-argument", `Attachment ${att.id} exceeds max size of ${MAX_FILE_SIZE_BYTES / (1024 * 1024)} MB`);
        }
        ...
        if (!att.path.startsWith(expectedPrefix)) {
            throw new functions.https.HttpsError("invalid-argument", `Invalid staging path for attachment ${att.id}`);
        }
        // Build final path
        ...
        try {
            ...
            if (!exists) {
                throw new functions.https.HttpsError("not-found", `Staging object not found for attachment ${att.id}`);
            }
            ...
            stagingFile.delete().catch((e) => {
                console.warn(`[commitStagedAttachments] Failed to delete staging ${att.path}: ${e.message}`);
            });
            ...
            if (error instanceof functions.https.HttpsError)
                throw error;
            console.error(`[commitStagedAttachments] Failed to commit ${att.id}:`, error);
            throw new functions.https.HttpsError("internal", `Failed to commit attachment ${att.id}`);
        }
    }
    return committed;
}
...
exports.mintChatMediaUrl = functions.https.onCall(async (data, context) => {
    ...
    if (!pathOk) {
        throw new functions.https.HttpsError("permission-denied", "Path does not belong to this conversation");
    }
    // Membership check
    const db = getDb();
    if (scope === "dm") {
    ...
    try {
        ...
        if (!exists) {
            throw new functions.https.HttpsError("not-found", "Media object not found");
        }
        ...
        console.log(`[mintChatMediaUrl] Signed URL minted for ${uid.substring(0, 8)}, path=${actualPath.substring(0, 40)}...`);
 ...
 */
exports.cleanupStagingOrphans = functions.pubsub
    .schedule("every 6 hours")
    .onRun(async () => {
    const bucket = admin.storage().bucket();
    const cutoff = Date.now() - STAGING_MAX_AGE_MS;
    console.log(`[cleanupStagingOrphans] Starting. Cutoff: ${new Date(cutoff).toISOString()}`);
    let deletedCount = 0;
    let errorCount = 0;
    try {
        ...
        for (const file of files) {
            try {
                ...
                errorCount++;
                console.warn(`[cleanupStagingOrphans] Failed to process ${file.name}:`, err);
            }
        }
    }
    catch (err) {
        console.error("[cleanupStagingOrphans] Failed to list staging files:", err);
    }
    console.log(`[cleanupStagingOrphans] Done. Deleted: ${deletedCount}, Errors: ${errorCount}`);
});
//# sourceMappingURL=chatMedia.js.map