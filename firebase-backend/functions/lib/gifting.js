...
exports.getGiftHistory = exports.expireGifts = exports.openGift = exports.sendGift = void 0;
...
function buildGiftHistoryRecord(params) {
    const { uid, type, giftId, itemId, itemName, itemImagePath, counterpartyUid, counterpartyName, message, purchasedAt, priceTokens, giftStatus, openedAt, } = params;
    return {
        type,
        userId: uid,
        itemId,
...
exports.sendGift = functions.https.onCall(async (data, context) => {
    ...
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be logged in to send gifts");
    }
    const senderUid = context.auth.uid;
    const { itemId, itemType, recipientUid, message, platform, purchaseToken, receiptData, transactionId, } = data;
    ...
    try {
        ...
        console.log(`[gifting] Gift created: ${giftRef.id} from ${senderUid} to ${recipientUid}`);
        const priceTokens = typeof productData?.priceTokens === "number"
        ...
        try {
            await (0, notificationCenter_1.notifyUser)({
                ...
                body: `You received ${itemName}. Tap to open it.`,
                actorUid: senderUid,
 ...
 */
exports.openGift = functions.https.onCall(async (data, context) => {
    // 1. Verify authentication
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be logged in to open gifts");
    }
    const uid = context.auth.uid;
    const { giftId } = data;
    if (!giftId) {
        throw new functions.https.HttpsError("invalid-argument", "Gift ID is required");
    }
    const db = admin.firestore();
    try {
        // Get gift
        ...
        if (productDoc.exists) {
            ...
            switch (gift.itemType) {
                ...
                case "bundle":
                    ...
                    itemsReceived = bundleItems.map((item) => item.itemId);
        ...
        console.log(`[gifting] Gift ${giftId} opened. Items: ${itemsReceived.length}, Tokens: ${tokensReceived}`);
        ...
        try {
            await (0, notificationCenter_1.notifyUser)({
                ...
                title: `${gift.recipientName} opened your gift!`,
                body: `They loved the ${gift.itemName}.`,
                actorUid: uid,
 ...
 * Runs hourly
 */
exports.expireGifts = functions.pubsub
