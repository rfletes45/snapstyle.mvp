...
exports.onCallCreated = functions.firestore
    ...
    const recipientIds = Object.keys(call.participants).filter((uid) => uid !== call.callerId);
...
// ============================================================================
function buildCallNotification(call, callId, recipient) {
...
// ============================================================================
async function sendCallCancelledNotification(callId, call) {
    // Get all participants except caller
    const recipientIds = Object.keys(call.participants).filter((uid) => uid !== call.callerId);
    // Get FCM tokens
    const tokenPromises = recipientIds.map(async (uid) => {
...
// Record Call History
// ============================================================================
async function recordCallHistory(callId, call) {
...
// ============================================================================
exports.handleCallTimeouts = functions.pubsub
...
// ============================================================================
exports.cleanupCallSignaling = functions.pubsub
...
exports.getTurnCredentials = functions.https.onCall(async (data, context) => {
    ...
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be logged in to get TURN credentials");
    }
...
const registerVoIPToken = functions.https.onCall(async (data, context) => {
    ...
    if (!voipToken || typeof voipToken !== "string") {
        throw new functions.https.HttpsError("invalid-argument", "VoIP token is required");
    }
    ...
    try {
        ...
        throw new functions.https.HttpsError("internal", "Failed to register VoIP token");
    }
});
// ============================================================================
...
const sendCallNotification = functions.https.onCall(async (data, context) => {
    ...
    if (!fcmToken && !voipToken) {
        throw new functions.https.HttpsError("failed-precondition", "Recipient has no push tokens");
    }
...
const cancelCall = functions.https.onCall(async (data, context) => {
    ...
    if (!cancellableStates.includes(call.status)) {
        throw new functions.https.HttpsError("failed-precondition", "Call cannot be cancelled in current state");
    }
    try {
...
const onGroupCallInviteCreated = functions.firestore
    ...
    const message = {
        ...
        notification: {
            title: invite.callType === "video" ? "📹 Group Video Call" : "📞 Group Call",
...
const onGroupCallParticipantJoined = functions.firestore
    ...
    const newlyJoined = afterParticipants.filter((afterP) => {
        ...
        return (afterP.joinedAt !== null && (!beforeP || beforeP.joinedAt === null));
    });
    ...
    const recipientIds = activeParticipantIds.filter((id) => !newlyJoined.some((p) => p.odId === id));
    if (recipientIds.length === 0) {
        return;
    }
    // Get FCM tokens
    ...
    const recipients = (await Promise.all(tokenPromises)).filter((r) => !!r.fcmToken);
    ...
    for (const joined of newlyJoined) {
        ...
        try {
            ...
            functions.logger.error("Failed to send participant joined notification", {
                callId,
...
const onGroupCallHostAction = functions.https.onCall(async (data, context) => {
    ...
    if (!callId || !action) {
        throw new functions.https.HttpsError("invalid-argument", "Call ID and action are required");
    }
    const userId = context.auth.uid;
    const callRef = db.collection("Calls").doc(callId);
    const callDoc = await callRef.get();
    ...
    if (!isHost && !isCoHost) {
        throw new functions.https.HttpsError("permission-denied", "Only host or co-host can perform this action");
    }
    // Handle different actions
    switch (action) {
        case "mute_all": {
            // Get all non-host participants to notify
            const recipientIds = Object.keys(call.participants).filter((id) => id !== userId);
        ...
        case "remove_participant": {
            if (!targetUserId) {
                throw new functions.https.HttpsError("invalid-argument", "Target user ID required for remove action");
            }
        ...
        case "end_call": {
            ...
            const allParticipantIds = Object.keys(call.participants).filter((id) => id !== userId);
...
// ============================================================================
async function sendGroupCallNotification(callId, recipientIds, payload) {
    if (recipientIds.length === 0)
        return;
    // Get FCM tokens
    const tokenPromises = recipientIds.map(async (uid) => {
