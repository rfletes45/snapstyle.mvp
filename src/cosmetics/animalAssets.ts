...
// =============================================================================
// Emoji ↔ Animal ID Mapping
// =============================================================================

/** Map animal ID → emoji (used when sending an animal message). */
...
/** Reverse map: emoji → animal ID (used when rendering received messages). */
export const EMOJI_ANIMAL_MAP: Record<string, string> = Object.fromEntries(
  Object.entries(ANIMAL_EMOJI_MAP).map(([id, emoji]) => [emoji, id]),
);

/** All known animal emojis (used for quick detection). */
export const ANIMAL_EMOJIS = new Set(Object.values(ANIMAL_EMOJI_MAP));

/**
 * Check if a message text is an animal emoji marker.
 * Returns the animal ID if yes, null otherwise.
 */
export function detectAnimalEmoji(text: string): string | null {
  const trimmed = text.trim();
  return EMOJI_ANIMAL_MAP[trimmed] ?? null;
}

/**
 * Get the emoji marker for an animal theme ID.
 * Falls back to duck emoji if unknown.
 */
export function getAnimalEmoji(animalId: string | null): string {
  return (
    ANIMAL_EMOJI_MAP[animalId ?? DEFAULT_ANIMAL_THEME_ID] ??
    ANIMAL_EMOJI_MAP[DEFAULT_ANIMAL_THEME_ID]
  );
}
