/**
 * Sketch Party — Word Bank
 *
 * Static word bank for Sketch Party. Words are categorized by difficulty
 * (easy/medium/hard) and tagged with length for scoring.
 *
 * Shared between client (for word display) and Colyseus server (for selection).
 *
 * @module gamesV4/data/sketchPartyWords
 */
...
/**
 * English word bank — curated for drawing games.
 * Keep words drawable and recognizable across cultures.
 */
...
/**
 * Select N random words from the bank, preferring a balanced mix.
 */
...
/**
 * Compute the masked word with hints revealed.
 * e.g., "butterfly" with 2 hints → "b _ _ t _ _ _ _ _"
 */
export function computeMaskedWord(word: string, hintsRevealed: number): string {
  ...
  // Reveal non-space characters deterministically (first char, last char, then spread)
 ...
 * Case-insensitive, ignores leading/trailing spaces and punctuation.
 */
