...
export default function StackPuzzleGameScreen({
  ...
  const renderBlock = (block: Block, index: number) => (
    ...
    >
      <Canvas style={{ width: block.width, height: BLOCK_HEIGHT }}>
        <RoundedRect x={0} y={0} width={block.width} height={BLOCK_HEIGHT} r={2}>
          <LinearGradient
            start={vec(0, 0)}
            end={vec(0, BLOCK_HEIGHT)}
            colors={[lightenHex(block.color, 30), block.color, darkenHex(block.color, 30)]}
          />
          <Shadow dx={0} dy={1} blur={2} color="rgba(0,0,0,0.25)" />
        </RoundedRect>
        {/* Top highlight */}
        <RoundedRect x={1} y={1} width={block.width - 2} height={3} r={1}>
          <LinearGradient
            start={vec(0, 1)}
            end={vec(0, 4)}
        ...
        {block.perfect && (
          <RoundedRect x={0} y={0} width={block.width} height={BLOCK_HEIGHT} r={2} color="rgba(255,215,0,0.2)" style="stroke" strokeWidth={2} />
        )}
      </Canvas>
    </View>
  );
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: colors.text }]}>Stack</Text>
      ...
      >
        ...
        {gameState === "playing" && (() => {
          const blockColor = COLORS[blocks.length % COLORS.length];
          const blockW = currentBlockWidth.current;
          return (
            <Animated.View
              style={[
                styles.block,
            ...
            >
              <Canvas style={{ width: blockW, height: BLOCK_HEIGHT }}>
                <RoundedRect x={0} y={0} width={blockW} height={BLOCK_HEIGHT} r={2}>
                  <LinearGradient
                    start={vec(0, 0)}
                    end={vec(0, BLOCK_HEIGHT)}
                    colors={[lightenHex(blockColor, 30), blockColor, darkenHex(blockColor, 30)]}
                  />
                  <Shadow dx={0} dy={2} blur={4} color="rgba(0,0,0,0.3)" />
                </RoundedRect>
                <RoundedRect x={1} y={1} width={blockW - 2} height={3} r={1}>
                  <LinearGradient
                    start={vec(0, 1)}
                    end={vec(0, 4)}
