...
export default function FlappyBirdGameScreen({
  ...
  const endGame = useCallback(async () => {
    ...
    if (currentFirebaseUser) {
      try {
        ...
        console.error("[FlappyBird] Error recording session:", error);
      }
    }
  }, [
    score,
  ...
  const handleSelectFriend = async (friend: {
    ...
    try {
      ...
      console.error("[FlappyBird] Error sharing score:", error);
      showError("Failed to share score. Try again.");
    } finally {
      setIsSending(false);
    }
          ...
          <Canvas style={StyleSheet.absoluteFill}>
            ...
            >
              <LinearGradient
                ...
                colors={["#0A1628", "#0F3460", "#16213E", "#1A1A2E"]}
              />
            </RoundedRect>
          </Canvas>
          ...
          {(status === "idle" || status === "gameOver") && (
            <View style={styles.overlay}>
              ...
              <Text style={styles.overlayTitle}>
                {status === "idle" ? "Flappy Bird" : "Game Over!"}
              </Text>
